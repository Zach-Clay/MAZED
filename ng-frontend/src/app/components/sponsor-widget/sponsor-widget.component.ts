      ...
      .then((user: any) => {
        this.userService.getUser(user.username).subscribe((data)=>{
          ...
          if (this.isSponsor) {
            ...
            this.applicationService.getApplicationsBySponsor(this.user.sponsorId).subscribe((apps) => {
              this.sponsorsApps = apps;
            })
          }
          
          if (this.isDriver) {
            this.applicationService.getApplicationsByUser(this.user.id).subscribe((apps) => {
              this.driverOutstandingApps = apps;
              this.driverOutstandingApps = this.driverOutstandingApps.filter(a => a.isActive === 1);
